...
public class OpenLineageSparkListenerTest {
  ...
  void testOpenlineageDisableDisablesExecution() throws URISyntaxException {
    try (MockedStatic mocked = mockStatic(Environment.class)) {
      ...
      listener.onJobStart(new SparkListenerJobStart(0, 2L, Seq$.MODULE$.<StageInfo>empty(), new Properties()));

      verify(contextFactory, never()).createSparkSQLExecutionContext(any());
    }
  }
}